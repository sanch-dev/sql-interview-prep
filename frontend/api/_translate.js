function adaptTSQL(sql) {
  ...
  out = out.replace(/\bDATEADD\s*\(\s*(year|month|day|hour|minute|second)\s*,\s*(-?\d+)\s*,\s*([^)]+)\)/gi, (_, part, n, date) => `(${date.trim()}::date + INTERVAL '${n} ${part}s')::date::text`)
  ...
  out = out.replace(/\bCONVERT\s*\(\s*(\w+(?:\s*\(\s*\d+\s*\))?)\s*,\s*([^,)]+)(?:,\s*[^)]+)?\)/gi, (_, type, expr) => `CAST(${expr.trim()} AS ${type.trim()})`)
  out = out.replace(/\bTRY_CAST\s*\(\s*([^A]+)\s+AS\s+(\w+(?:\s*\(\s*\d+\s*\))?)\s*\)/gi, (_, expr, type) => `CAST(${expr.trim()} AS ${type.trim()})`)
  out = out.replace(/\bSTUFF\s*\(\s*([^,]+),\s*([^,]+),\s*([^,]+),\s*([^)]+)\)/gi, (_, str, start, len, repl) => `OVERLAY(${str.trim()} PLACING ${repl.trim()} FROM ${start.trim()} FOR ${len.trim()})`)
